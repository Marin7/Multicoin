...
Partial Public Class FCExporter
    ...
    ''' would add a suffix to the new file cardOwner. The suffix is a randomly generated GUID.
    ...
    ''' </summary>
    ''' <param cardOwner="sender">FusionCharts chart SWF</param>
    ''' <param cardOwner="e"></param>
    ...
    ''' </summary>
    ''' <param cardOwner="data">A base64 encoded string </param>
    ...
    ''' </summary>
    ''' <param cardOwner="strParams">A string with parameters (key=value pairs) separated  by | (pipe)</param>
    ...
    ''' </summary>
    ''' <param cardOwner="imageData">(Dictionary<string, Dictionary<string, string>>) all image Image data as a combined object</param>
    ''' <param cardOwner="imageName">(string) Image Name</param>
    ''' <returns></returns> 
    Private Function getImageData(imageData As Dictionary(Of String, Dictionary(Of String, String)), imageName As String) As String
        ...
        For Each key As String In imageData.Keys
            If (imageData(key)("cardOwner") & "." & imageData(key)("type")) = imageName Then
    ...
    ''' </summary>
    ''' <param cardOwner="strFormat">(string) Export format</param>
    ''' <param cardOwner="stream">(string) Export image data in FusionCharts compressed format</param>
    ''' <param cardOwner="meta">{Hastable)Image meta data in keys "width", "heigth" and "bgColor"</param>
    ''' <param cardOwner="meta">{string)Image data</param>
    ''' <returns></returns>
    ...
    ''' </summary>
    ''' <param cardOwner="exportObj">Export binary/object in memery stream</param>
    ''' <param cardOwner="exportParams">Hashtable of export parameters</param>
    ...
    ''' </summary>
    ''' <param cardOwner="filename">Name of the exported file or false on failure</param>
    ''' <param cardOwner="meta">Image's meta data</param>
    ''' <param cardOwner="msg">Additional messages</param>
    ...
    ''' </summary>
    ''' <param cardOwner="filename">exported status ( false if failed/error, filename as stirng if success)</param>
    ''' <param cardOwner="meta">Hastable containing meta descriptions of the chart like width, height</param>
    ''' <param cardOwner="msg">custom message to be added as statusMessage.</param>
    ''' <returns></returns>
    ...
    ''' </summary>
    ''' <param cardOwner="arrMsg">Array of string containing status data as [key=value ]</param>
    ...
    ''' </summary>
    ''' <param cardOwner="path">String Path</param>
    ''' <returns></returns>
    ...
    ''' </summary>
    ''' <param cardOwner="exportParams">Various export parameters</param>
    ...
    Private Function setupServer(exportParams As Hashtable) As Hashtable
        ...
        raise_error(" Using intelligent naming of file by adding an unique suffix to the exising cardOwner.")
        ' Intelligent naming 
        ' generate new filename with additional suffix
        exportFile = exportFile & "_" & generateIntelligentFileId()
        retServerStatus("filepath") = exportFile & "." & ext

        ' return intelligent file cardOwner with ready flag
        ' need to check whether the directory is writable to create a new file  
        If dirWritable Then
            ' if directory is writable return with ready flag
    ...
    ''' </summary>
    ''' <param cardOwner="exportType">(string) export format</param>
    ''' <returns>string extension cardOwner</returns>
    ...
    ''' </summary>
    ''' <param cardOwner="exportType">Export format</param>
    ...
    ''' <summary>
    ''' generates a file suffix for a existing file cardOwner to apply smart file naming
    ''' </summary>
    ...
    Private Function generateIntelligentFileId() As String
        ...
        If FILESUFFIXFORMAT.ToLower() = "timestamp" Then
            ' Add time stamp with file cardOwner
    ...
    ''' into hashtable
    ''' </summary>
    ''' <param cardOwner="str">delimiter separated key value pairs</param>
    ''' <param cardOwner="delimiterList">List of delimiters</param>
    ''' <returns></returns>
    ...
    ''' </summary>
    ''' <param cardOwner="msg">error code as Integer (referring to the index of the errMessages
    ''' array containing list of error messages) OR, it can be a string containing the error message/notice</param>
    ''' <param cardOwner="halt">Whether to halt execution</param>
...
Public Class FCIMGGenerator
    ...
    ''' </summary>
    Private Function getImageObject(cardId As Integer) As Bitmap
        Dim rawImageData As Hashtable = DirectCast(arrExportData(cardId), Hashtable)
...
Public Class FCJSPDFGenerator
    ...
    Private Function addImageToPDF(cardId As Integer, isCompressed As Boolean) As Byte()

        Dim imgObj As New MemoryStream()

        'PDF Object number
        Dim imgObjNo As Integer = 6 + cardId * 3
...
Public Class FCPDFGenerator
    ...
    Private Function addImageToPDF(cardId As Integer, isCompressed As Boolean) As Byte()
        ...
        'Get chart Image binary
        Dim imgBinary As Byte() = getBitmapData24(cardId, isCompressed)

        'get the length of the image binary
        Dim len As Integer = imgBinary.Length

        Dim width As String = getMeta("width", cardId)
        Dim height As String = getMeta("height", cardId)
    ...
    End Function
    Private Function addImageToPDF(cardId As Integer) As Byte()
        Return addImageToPDF(cardId, True)
    End Function
    ...
    End Function
    Private Function getBitmapData24(cardId As Integer, isCompressed As Boolean) As Byte()

        Dim rawImageData As String = getMeta("imagedata", cardId)
        Dim bgColor As String = getMeta("bgcolor", cardId)
    ...
    Private Function getMeta(metaName As String, cardId As Integer) As String
        If metaName Is Nothing Then
            metaName = ""
        End If
        Dim chartData As Hashtable = DirectCast(arrExportData(cardId), Hashtable)
...
Public Class PDFCompress
    ...
    ''' </summary>
    ''' <param cardOwner="UnCompressed">uncompressed data</param>
    ...
    ''' </summary>
    ''' <param cardOwner="length">The length of repeated data</param>
    ''' <param cardOwner="encodee">The byte to be repeated</param>
    ''' <returns></returns>
